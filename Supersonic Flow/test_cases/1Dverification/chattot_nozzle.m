...
xx = linspace(0.5*(1-sqrt(3)/3),1,201);
dx = xx(2) - xx(1);
...
% UU(1,2:end) = g_x(2:end).*(gam.*p_i).^(1/gam);%2*gam.*p_i./(gam-1).*g_x(2:end);
% UU(2,2:end) = 0;
% UU(3,2:end) = p_i.*g_x(2:end)/(gam-1);
UU = repmat(UU,1,1,3);

% Setup Time Simulation?
time = 0;
tEnd = 10*5;
[flux, Umax, ~, visc_beta] = fx_2Diff(@fluxFunc, @epsConst, UU(:,:,end), g_x, gam, dx);
