...
eps_s = 0.0025; % spatial diffusion term
...
EE.fv = repmat(cat(3,   ones(size(GR.XX)),... % rho
                        ...
                        ones(size(GR.XX)).^(gam) ./ (gam .* M0.^2)),... % P ... eps_mach error here?
                        1, 1, 1, 3); 
...
while isempty(res) || ((max(res(end, :)) > tol*max(res(res<1)))|| (size(res,1) < iter_min)) % iterate through time
    ...
    %% Update Field Values
    EE.fv(:,:,:,1:2) = EE.fv(:,:,:,2:3);
    EE.fv(:,:,4,2) = EE.fv(:,:,1,2).^(gam) ./ (gam .* M0.^2);
    FF.fv = repmat(EE.fv(:,:,2,2)./EE.fv(:,:,1,2),1,1,size(EE.fv,3)) .* EE.fv(:,:,:,2);
    GG.fv = repmat(EE.fv(:,:,3,2)./EE.fv(:,:,1,2),1,1,size(EE.fv,3)) .* EE.fv(:,:,:,2);
    ...
    FF.fx(:,end,:) = repmat(reshape(cell2mat(BC.E(:,1)),1,1,4),size(FF.fx,1),1,1);
    FF.fx(:,1,:) = (FF.fv(:,2,:) - BC.W{2,1}./BC.W{1,1}.*repmat(reshape(cell2mat(BC.W(:,1)),1,1,4),size(FF.fx,1),1,1))./(2*GR.dx);
    ...
    EE.fx_b(:,1,:) = (EE.fv(:,1,:,2) - repmat(reshape(cell2mat(BC.W(:,1)),1,1,4),size(EE.fv,1),1,1))./(GR.dx); % left boundary
    ...
    EE.laplace = (EE.fx_f(:,:,1:3) - EE.fx_b(:,:,1:3))./(GR.dx) + (EE.fy_f(:,:,1:3) - EE.fy_b(:,:,1:3))./(GR.dy);
    ...
    pTerms = cat(3, zeros(size(GR.XX)),...
                        EE.fx(:,:,end),...
                        EE.fy(:,:,end));
    EE.fv(:,:,1:3,3) = (eps_s.*(EE.laplace + DF.*EE.fv(:,:,1:3,2)) - 0.5*EE.fv(:,:,1:3,1).*(eps_s*DF - 1/dt) - (FF.fx(:,:,1:3) + GG.fy(:,:,1:3) + pTerms))./(0.5/dt + 0.5*eps_s*DF);
    EE.fv(stepInd{1}, stepInd{2},:,:) = NaN;
    ...
    if (size(res,1) == 1) && all(res(end,:) == 0)
        res(1,:) = reshape(max(max(abs(EE.fv(:,:,1:3,3) - EE.fv(:,:,1:3,2)))),1,3,1);
    else
        res(end+1,:) = reshape(max(max(abs(EE.fv(:,:,1:3,3) - EE.fv(:,:,1:3,2)))),1,3,1);
    end
    ...
    if (abs(4 - tot_time) < dt) && (tot_time > 4)
        saveas(gcf, [case_name '_' num2str(dx) '_density.pdf']);
    end
