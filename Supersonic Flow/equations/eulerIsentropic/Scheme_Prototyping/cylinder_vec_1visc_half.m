...
case_name = 'cylinder_vec_1visc_half_test';
...
% load('cylinder_vec_1visc_half_test50\372_91_0.133_0.017\M_1.4\results.mat','EE');
...
outletInd = find(GR.XX(end,:)>=R_range(end).*dT); % index for north outlet
inletInd = find(~(GR.XX(end,:)>=R_range(end).*dT));
...
%        fprintf('Decreasing time steps!\n');
        cflFactor = min(1./CFL_i, 1/max(abs(alphaTot(:))));
        dt = dt.*cflFactor;
%        fprintf('New time step:%0.5f\n', dt);
    ...
    if M0 >= 1
        FF.grad_Rf = cat(1, diff(FF.fv,1,1)./dr,([FF.fv(end,outletInd,:), cat(3, rA.BC.N(end,inletInd), rA2_R.BC.N(end,inletInd), rAB_R.BC.N(end,inletInd))]  - FF.fv(end,:,:))./dr);
    else
...
%     FF.grad_Rb = cat(1, (FF.fv(1,:,:) - FF.fv(1,:,:))./(0.5*dr), diff(FF.fv,1,1)./dr);
    FF.grad_R = 0.5.*(FF.grad_Rf + FF.grad_Rb);

    
    %% Calculate Angular Derivatives
    % GG contains B, AB/Rho, B^2/Rho
...
%     GG.BC.Tb = (-2.*GG.fv(:,1,:)+3.*GG.fv(:,2,:) - GG.fv(:,3,:))./dT;
    GG.BC.Tb = (GG.fv(:,1,:) - (4/3.*GG.fv(:,1,:)-1/3.*GG.fv(:,2,:)))./dT;
    GG.grad_T = 0.5.*([diff(GG.fv,1,2)./dT, GG.BC.Tf] + [GG.BC.Tb, diff(GG.fv,1,2)./dT]);
    
    %% Calculate Laplacian
    if M0 >= 1
        EE.grad_Rf = cat(1, diff(EE.fv(:,:,:,2),1,1)./dr, ([EE.fv(end,outletInd,:,2).*GR.RR(end,outletInd)./(GR.RR(end,outletInd)+dr.*cat(3,0,1,1)), cat(3, RHO.BC.N(end,inletInd), AA.BC.N(end,inletInd), BB.BC.N(end,inletInd))] - EE.fv(end,:,:,2))./dr);
    else
