...
%     [A_r, A_rf, A_rb] = grad_f(GR.RR.*AA.fv(:,:,2), 1, dr, rA.BC, 1);
    ...
    rA.grad_Rb = [(rA.fv(1,:) - rA.BC.S)./(0.5*dr); diff(rA.fv, 1, 1)./dr]; % reflective boundary condition
...
%     RHO.grad_RR = (rr_n.* RHO.grad_Rf - rr_s.*RHO.grad_Rb)./(dr .* GR.RR);
    ...
    RHO.fv(:,:,3) = (eps_s.*((rr_n.*(RHO.grad_Rf.*dr+RHO.fv(:,:,2)) + rr_s.*(RHO.grad_Rb.*dr+RHO.fv(:,:,2)))./(GR.RR .* dr^2)...
                                + (RHO.grad_Tf.*dT + RHO.grad_Tb.*dT - 2.*RHO.fv(:,:,2))./(GR.RR.^2 .* dT.^2))...
                    -rA.grad_R./GR.RR - BB.grad_T./GR.RR...
                    -(eps_s.*((rr_n + rr_s)./(2.*GR.RR.*dr.^2) + 1./(GR.RR.^2 .* dT.^2)) - 1./(2*dt)).*RHO.fv(:,:,1))./...
                    (0.5/dt + eps_s.*((rr_n + rr_s)./(2.*GR.RR.*dr.^2) + 1./(GR.RR.^2 .* dT.^2)));
...
%     [AB_rho_T, ~, ~] = grad_f((AA.fv(:,:,2).*BB.fv(:,:,2))./RHO.fv(:,:,2), 2, dT, AB_R.BC, 1);
    ...
    AB_R.grad_Tf = [diff(AB_R.fv, 1,2)./dT, (-AB_R.fv(:,end-1) - AB_R.fv(:,end))./dT]; % mirrored/symmetry BC
