...
while (res(end) > tol)|| (length(res) < iter_min) % iterate through time
    ...
    if M0 ~= 0
        ...
        % Laplace of PHI
        phi_xx = (phiX_e - phiX_w)./dx + 2.*PHI(:,:,2)./(dx^2);
        phi_yy = (phiY_n - phiY_s)./dy + 2.*PHI(:,:,2)./(dy^2);
        
         % Laplace of RHO
        rho_xx = (rhoX_e - rhoX_w)./dx + 2.*RHO(:,:,2)./(dx^2);
        rho_yy = (rhoY_n - rhoY_s)./dy + 2.*RHO(:,:,2)./(dy^2);
...
%             dt = dt / CFL_i;
        ...
        RHO(:,:,3) = -(mflux_ss + (eps_1*(1/(dx^2) + 1/(dy^2)) - 0.5/dt).*RHO(:,:,1) - eps_1.*(rho_xx + rho_yy))./(eps_1*(1/(dx^2) + 1/(dy^2)) + 0.5/dt);
...
%         end
        ...
        PHI(:,:,3) = -(((RHO(:,:,2).^(gam-1) - 1)./((gam-1)*M0^2) + 0.5.*(q2_ij - 1)) + (eps_2*(1/(dx^2) + 1/(dy^2)) - 0.5/dt).*PHI(:,:,1) - eps_2.*(phi_xx + phi_yy))./(eps_2*(1/(dx^2) + 1/(dy^2)) + 0.5/dt);
...
%         end
    ...
    if ((res(end) > res(end-1)) && (res(end) < (5*tol))) && (length(res) > iter_min)
...
if ~exist([pwd '\euler_cyl\' folderName], 'dir')
    mkdir([pwd '\euler_cyl\' folderName]);
end
...
saveas(gcf, [pwd '\euler_cyl\' folderName '\residual_plot.png']);
saveas(gcf, [pwd '\euler_cyl\' folderName '\residual_plot']);

% plot density
figure();contourf(XX,YY,round(RHO(:,:,3),3), 50)
title('Density (Normalized)');
colorbar('eastoutside');
axis equal
saveas(gcf, [pwd '\euler_cyl\' folderName '\density.png']);
saveas(gcf, [pwd '\euler_cyl\' folderName '\density']);
...
colorbar('eastoutside');
axis equal
saveas(gcf, [pwd '\euler_cyl\' folderName '\cp_contour.png']);
saveas(gcf, [pwd '\euler_cyl\' folderName '\cp_contour']);
...
saveas(gcf, [pwd '\euler_cyl\' folderName '\cp_surf.png']);
saveas(gcf, [pwd '\euler_cyl\' folderName '\cp_surf']);
...
colorbar('eastoutside');
axis equal
saveas(gcf, [pwd '\euler_cyl\' folderName '\phi_pot.png']);
saveas(gcf, [pwd '\euler_cyl\' folderName '\phi_pot']);

figure(); % theta-dir velocity plots
contourf(XX, YY, round(phiX,5), 50); %./((RR.*cos(TT)).^2)
title('\Phi_\theta velocity');
colorbar('eastoutside');
saveas(gcf, [pwd '\euler_cyl\' folderName '\phi_theta.png']);
saveas(gcf, [pwd '\euler_cyl\' folderName '\phi_theta']);

figure(); % theta-dir velocity plots
contourf(XX, YY, phiY, 50); %./((RR.*cos(TT)).^2)
title('\Phi_R velocity');
colorbar('eastoutside');
saveas(gcf, [pwd '\euler_cyl\' folderName '\phi_radius.png']);
saveas(gcf, [pwd '\euler_cyl\' folderName '\phi_radius']);
...
% colorbar('eastoutside');
% axis equal
% saveas(gcf, [pwd '\euler_cyl\' folderName '\mach.png']);
% saveas(gcf, [pwd '\euler_cyl\' folderName '\mach']);

%% Save Results
save([pwd '\euler_cyl\' folderName '\results.mat']);
