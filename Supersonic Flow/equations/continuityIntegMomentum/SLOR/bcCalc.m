...
%     end
    ...
    switch DIR
        ...
        case 'E' 
            ...
            BC_vals = 2./3.*(2.*FF(:,end-1,:)-0.5.*FF(:,end-2,:)).*indTan + 0.5.*(5.*FF(:,end-1,:)-4.*FF(:,end-2,:)+FF(:,end-3,:)).*indPhi;
...
function BC_vals = gen_wallBC(GR, BC, FF, DIR)
    ...
    switch DIR
        ...
        case 'S'
            ...
            indPhi = reshape(strcmp(BC.(DIR).varType, 'phi'),1,1,size(FF,3)); % checks for potential
            indNorm = reshape(strcmp(BC.(DIR).varType, 'v1'),1,1,size(FF,3));
            phiX = (FF(:,3:end,2) - FF(:,1:end-2,2))./(2.*GR.dx);
            BC_vals = 2./3.*(2.*FF(2,:,:) - 0.5.*FF(3,:,:)... .*(indTan+indScalar)...
                            - [1, phiX(1,:), phiX(1,end)] .* GR.dy.*reshape(BC.(DIR).dydx,1,size(FF,2)).*indPhi);... - FF(1,:,:);
%             BC_vals = (FF(2,:,:)... .*(indTan+indScalar)...
