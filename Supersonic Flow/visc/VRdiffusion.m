...
f2 = epsE.*[EE(:,2:end,:),bcCalc(GR, FL, BC, EE(:,:,:), 'E')]  + epsW.*[bcCalc(GR, FL, BC, EE(:,:,:), 'W'), EE(:,1:end-1,:)];
if GR.isPolar
    ...
    if any(indV1) || any(indV2)
        rotLaplace =- indV1.*(EE(:,:,indV1) +...
                            2.*([EE(:,2:end,indV2),bcCalc(GR,FL,BC,EE(:,:,:),'E',find(indV2))] - [bcCalc(GR,FL,BC,EE(:,:,:),'W',find(indV2)),EE(:,1:end-1,indV2)])./(2.*GR.dT))./(GR.RR.^2)...
                - indV2.*(EE(:,:,indV2) -...
                            2.*([EE(:,2:end,indV1),bcCalc(GR,FL,BC,EE(:,:,:),'E',find(indV1))] - [bcCalc(GR,FL,BC,EE(:,:,:),'W',find(indV1)),EE(:,1:end-1,indV1)])./(2.*GR.dT))./(GR.RR.^2);
    else
        rotLaplace = 0;
    end
    alpha2 = 2.*GR.dt.*epsFunc(GR,FL,BC,'T')./(GR.RR.*GR.dT).^2;
    alpha1 = 2.*GR.dt.*epsFunc(GR,FL,BC,'R')./(GR.RR.*GR.dR.^2).*0.5.*(GR.RR_N+GR.RR_S);
else
    % Compute diffusion terms at time n
    f1 = epsN.*[EE(2:end,:,:); bcCalc(GR,FL,BC, EE(:,:,:),'N')] + epsS.*[bcCalc(GR, FL, BC, EE(:,:,:),'S'); EE(1:end-1,:,:)];
