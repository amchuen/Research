...
while isempty(res) || ((max(res(end, :)) > tol*max(res(res<1)))|| (size(res,1) < iter_min)) % iterate through time
    ...
    if M0 > 1 % Neumann condition for supersonic flow
        ...
        EE.fx3_f(:,end-1:end,:) = diff([EE.fv(:,end-2:end,:,2), EE.fv(:,end-1:end,:,2)],3,2);
        pSX = [repmat(BC.W{5,1},size(EE.fv,1),2), EE.fv(:,:,5,2), EE.fv(:,end-1:end,5,2)]; %->x-dir pressure sensor
        ...
        EE.fx3_f(:,end-1:end,:) = diff([EE.fv(:,end-2:end,:,2), repmat(reshape(cell2mat(BC.E(:,1)),1,1,5),size(EE.fv,1),2,1)],3,2);
        pSX = [repmat(BC.W{end,1},size(EE.fv,1),2), EE.fv(:,:,5,2), repmat(BC.E{end,1},size(EE.fv,1),2)]; %->x-dir pressure sensor
    end
    ...
    % Third Derivatives
    EE.fx3_f(:,1:end-2,:) = diff([repmat(reshape(cell2mat(BC.W(:,1)),1,1,5),size(EE.fv,1),1,1), EE.fv(:,:,:,2)],3,2);
    EE.fx3_b(:,end,:) = EE.fx3_f(:,end-1,:);
    EE.fx3_b(:,1:end-1,:) = diff([repmat(reshape(cell2mat(BC.W(:,1)),1,1,5),size(EE.fv,1),2,1), EE.fv(:,:,:,2)],3,2);
    nuX = abs(diff(pSX,2,2))./(abs(pSX(:,3:end)) + 2.*abs(pSX(:,2:end-1)) + abs(pSX(:,1:end-2)));
    
    %% Calculate Y Derivative
    refl = reshape([1,1,-1,1,1],1,1,5);
    ...
    % Third Derivatives
    EE.fy3_f = diff([refl.*EE.fv(1,:,:,2); EE.fv(:,:,:,2); repmat(reshape(cell2mat(BC.N(:,1)),1,1,5),2,size(EE.fv,2),1)],3,1);
    EE.fy3_b = diff([refl.*EE.fv(2:-1:1,:,:,2); EE.fv(:,:,:,2); repmat(reshape(cell2mat(BC.N(:,1)),1,1,5),1,size(EE.fv,2),1)],3,1);
    pSY = [EE.fv(2:-1:1,:,5,2); EE.fv(:,:,5,2); repmat(BC.W{end,1},2,size(EE.fv,2))];
    nuY = abs(diff(pSY,2,1))./(abs(pSY(3:end,:)) + 2.*abs(pSY(2:end-1,:)) + abs(pSY(1:end-2,:)));
    
    %% Calculate Dissipation
    
    eps2fx = repmat(k2.*max(nuX(:,3:end), nuX(:,2:end-1)),1,1,4);     eps4fx = max(0, k4-eps2fx);
    eps2bx = repmat(k2.*max(nuX(:,2:end-1), nuX(:,1:end-2)),1,1,4);   eps4bx = max(0, k4-eps2bx);
    eps2fy = repmat(k2.*max(nuY(3:end,:), nuY(2:end-1,:)),1,1,4);     eps4fy = max(0, k4-eps2fy);
    eps2by = repmat(k2.*max(nuY(2:end-1,:), nuY(1:end-2,:)),1,1,4);   eps4by = max(0, k4-eps2by);
    
    EE.dissipation =    (((eps2fx.*EE.fx_f(:,:,1:4).*GR.dx-eps4fx.*EE.fx3_f(:,:,1:4))-(eps2bx.*EE.fx_b(:,:,1:4).*GR.dx-eps4bx.*EE.fx3_b(:,:,1:4)))... x-dissipation
                        +((eps2fy.*EE.fy_f(:,:,1:4).*GR.dy-eps4fy.*EE.fy3_f(:,:,1:4))-(eps2by.*EE.fy_b(:,:,1:4).*GR.dy-eps4by.*EE.fy3_b(:,:,1:4))))./dt;... y-dissipation
    
    %% Calculate Next Time Step
    
%     DF = (eps2fx+eps2bx) + (eps2fy+eps2by); % Dufort-Frankel coefficient
    pTerms = cat(3, zeros(size(GR.XX)),...
                        EE.fx(:,:,end),...
                        EE.fy(:,:,end),...
                        FF.fx(:,:,end) + GG.fy(:,:,end));
%     EE.fv(:,:,1:4,3) = (EE.dissipation + DF.*EE.fv(:,:,1:4,2) - 0.5*EE.fv(:,:,1:4,1).*(DF - 1/dt) - (FF.fx(:,:,1:4) + GG.fy(:,:,1:4) + pTerms))./(0.5/dt + 0.5.*DF);
    EE.fv(:,:,1:4,3) = 2.*dt.*(EE.dissipation - FF.fx(:,:,1:4) - GG.fy(:,:,1:4) - pTerms) + EE.fv(:,:,1:4,1);
