...
eps_s = 0.0075;%2^-11; % spatial diffusion term
...
while isempty(res) || ((max(res(end, :)) > tol*max(res(res<1)))|| (size(res,1) < iter_min)) % iterate through time
    ...
    EE.fx_f(:,1:end-1,:) = EE.fv(:,2:end,:,2); % forward difference
    EE.fx_b(:,2:end,:) = EE.fv(:,1:end-1,:,2); % backward difference
    if M0 > 1 % Neumann condition for supersonic flow
        EE.fx_f(:,end,:) = EE.fv(:,end-1,:,2); % right boundary
    else % Dirichlet/Free-Stream Condition for Subsonic FLow
        EE.fx_f(:,end,:) = repmat(reshape(cell2mat(BC.E(:,1)),1,1,5),size(EE.fx,1),1,1); % right boundary
    end
    EE.fx_b(:,1,:) = repmat(reshape(cell2mat(BC.W(:,1)),1,1,5),size(EE.fv,1),1,1); % left boundary
    Px = (EE.fx_f(:,:,end) - EE.fx_b(:,:,end))./(2.*GR.dx); % central diff -> extract P_x
    epsfx = eps_s.*(abs(EE.fx_f(:,:,1) - EE.fv(:,:,1,2)).^2)./(0.5.*(abs(EE.fx_f(:,:,1))+ abs(EE.fv(:,:,1))));
    epsbx = eps_s.*(abs(EE.fv(:,:,1,2) - EE.fx_b(:,:,1)).^2)./(0.5.*(abs(EE.fx_b(:,:,1))+ abs(EE.fv(:,:,1))));
    ...
    EE.fy_f(1:end-1,:,:) = EE.fv(2:end,:,:,2);
    EE.fy_b(2:end,:,:) = EE.fv(1:end-1,:,:,2);
    EE.fy_f(end,:,:) = repmat(reshape(cell2mat(BC.N(:,1)),1,1,5),1,size(EE.fv,2),1);
    EE_S = EE.fv(1,:,:,2);
    EE_S(:,:,3) = BC.S{3,1}.* EE.fv(1,:,1,2);
    EE.fy_b(1,:,:) = EE.fv(1,:,:,2) - (EE.fv(1,:,:,2) - EE_S)./(0.5);
    Py = (EE.fy_f(:,:,end) - EE.fy_b(:,:,end))./(2.*GR.dy);
    
    epsfy = eps_s.*(abs(EE.fy_f(:,:,1) - EE.fv(:,:,1,2)).^2)./(0.5.*(abs(EE.fy_f(:,:,1)) + abs(EE.fv(:,:,1))));
    epsby = eps_s.*(abs(EE.fv(:,:,1,2) - EE.fy_b(:,:,1)).^2)./(0.5.*(abs(EE.fv(:,:,1)) + abs(EE.fy_b(:,:,1))));
    
    %% Calculate Next Time Step
    laplacian = (epsfx.*(EE.fx_f(:,:,1:4) - EE.fv(:,:,1:4,2)) - epsbx.*(EE.fv(:,:,1:4,2) - EE.fx_b(:,:,1:4)))./(GR.dx^2) + (epsfy.*(EE.fy_f(:,:,1:4) - EE.fv(:,:,1:4,2)) - epsby.*(EE.fv(:,:,1:4,2) - EE.fy_b(:,:,1:4)))./(GR.dy^2);
    
    DF = repmat(((epsfx+epsbx)./(GR.dx^2) + (epsfy+epsby)./(GR.dy^2)),1,1,4); % Dufort-Frankel coefficient
    pTerms = cat(3, zeros(size(GR.XX)),...
                        Px,...
                        Py,...
                        FF.fx(:,:,end) + GG.fy(:,:,end));
    EE.fv(:,:,1:4,3) =  ((1-dt.*DF).*EE.fv(:,:,1:4,1) +...
                        2.*dt.*((repmat(epsfx,1,1,4).*EE.fx_f(:,:,1:4)+repmat(epsbx,1,1,4).*EE.fx_b(:,:,1:4))./(GR.dx^2) + (repmat(epsfy,1,1,4).*EE.fy_f(:,:,1:4)+repmat(epsby,1,1,4).*EE.fy_b(:,:,1:4))./(GR.dy^2)) ...
                        - 2.*dt.*(FF.fx(:,:,1:4) + GG.fy(:,:,1:4) + pTerms))./(1 + dt.*DF);
