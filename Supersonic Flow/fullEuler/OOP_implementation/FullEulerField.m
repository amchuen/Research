classdef FullEulerField < handle
% NOTES:
...
%     - numerical diffusion is introduced to form the shockwave
    ...
    methods % methods for creating the object and setting up the field
        function obj = FullEulerField(ctrl, grid_lims, fluid_params, BC_setup)
            ...
            obj.initField(grid_lims); % initialize the grid?
            obj.initBC(BC_setup.phys_types, BC_setup.vals, BC_setup.ranges); % initialize boundary conditions?
            obj.init_fv;
        end 
        ...
        function obj = initField(obj, grid_lims)
            ...
            % 1:Y, 2:X, 3:vec, 4:t
             ...
             EE.fv = ones([size(obj.GR.D1), 5,3]);
             %repmat(cat(3,   ones(size(obj.GR.D1)),... % rho
%                                     ones(size(obj.GR.D1)),... % rho * u
%                                     ones(size(obj.GR.D1)),...% rho * v
%                                     ones(size(obj.GR.D1))./(obj.FL.gam*(obj.FL.gam-1)*obj.FL.M0^2) + 0.5,... % rho * E
%                                     1/(obj.FL.gam * obj.FL.M0^2).*ones(size(obj.GR.D1))),... % P ... eps_mach error here?
%                                     1, 1, 1, 3); 
%             
        end
        
        function obj = initBC(obj, phys_types, vals, range)
            % inputs:
...
%             end
%             out = any(out);
            out = isempty(whole_res) || ((max(whole_res(end,:)) > obj(1).CT.tol*max(whole_res(whole_res<1)))|| (size(whole_res,1) < obj(1).CT.iter_min));
        end
        ...
        function obj = updateVals(obj)
            ...
            for ii = 1:length(obj)
                % update each grid's boundary conditions
                obj(ii) = obj(ii).updateBC_tl(obj((1:length(obj)) ~= ii));

                % update each vector's derivatives
                for i = 1:length(obj(ii).FV)
                    obj(ii).FV(i) = obj(ii).FV(i).Gradient();
                    obj(ii).FV(i) = obj(ii).FV(i).Laplacian();
                end
                
                % update pressure derivatives
                obj(ii).PP = obj(ii).PP.Gradient();
                obj(ii).PP = obj(ii).PP.Laplacian();
            end               
        end
        ...
        function obj = updatePatchBC(obj, others, DIR, bcIdx)
            ...
            for fv = 1:length(others)
                ...
                for ii = 1:length(others(fv).BC(1).(test_dir)) % loop through other grid's BC ranges
                    ...
                    if (test_home == test_patch) && all(home_range == patch_range) % i.e. node point ranges match
                        bf = 1;
                        break;
                    end
                end
        ...
        function obj = CFLcheck(obj)
            ...
            if CFL_i >= obj(1).CT.maxCFL
               fprintf('CFL condition not met!\n');
               
               if obj(1).CT.CFL_on
                   fprintf('Decreasing time steps!\n');
                   for i = 1:length(obj)
                        obj(i).CT.dt = obj(i).CT.dt*0.8*obj(i).CT.maxCFL / CFL_i;
                   end
